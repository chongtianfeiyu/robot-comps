package
{
	...
	public class KitchenSink
	{
		...
		public function KitchenSink(root:*, stage:Stage){
			...
			//Add BG
			bg = Display.getImageFromBitmap(new BgStrip());
			bg.width = width;
			bg.height = height;
			root.addChild(bg);
		...
		protected function onStageResized(width:int, height:int):void {
			...
			themeChooser.x = width - themeChooser.width - 10;
			menu.y = KitchenSink.height - menu.height;
		...
		protected function createMenu():void {
			...
			for(var i:int = 0, l:int = menuItems.length; i < l; i++){
				...
				var button:SimpleButton = new SimpleButton(menuItems[i], null, Display.BG_LIGHT);
